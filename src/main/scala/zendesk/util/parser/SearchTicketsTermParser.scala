...
import zendesk.util.parser.SearchTicketsTerm._

object SearchTicketsTermParser {
  ...
  def parseTerm[_: P]: P[SearchTicketsTerm] = P(
    parseId
      | parseUrl
      | parseExternalId
      | parseType
  ...
  def doParse(command: String): Either[AppError, SearchTicketsTerm] = {
